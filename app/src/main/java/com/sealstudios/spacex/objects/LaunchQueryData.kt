...
data class LaunchQueryData(
    ...
    companion object {
        ...
        fun getLaunchSuccessFromQuery(launchQueryData: LaunchQueryData): Boolean {
            val success =  launchQueryData.query?.entries?.firstOrNull { entry -> entry.key == "success" }?.value
            return if (success != null) success as Boolean else false
        }

        fun isSortOrderAscending(launchQueryData: LaunchQueryData): Boolean {
            return launchQueryData.options?.sort?.entries?.first() ?: "desc" == "asc"
        }
